...
| 10 | 笔 | **成笔失败后怎么办** | 取极值（第 77 课原文程序）/ 纯丢弃 / 回退 | 第 8 章 |
...
2. **它几乎从不出现在任何文档里**（第 8 章 Q6）。它通常藏在
   ...
   ⚠️ 而**原文其实给过完整程序**——第 77 课在证明笔划分唯一性之后，
   直接给出了三步划分步骤，实质就是**同类型取极值**。
...
| "只要按原文实现就不会有分歧" | ⚠️ 有争议 | 原表述在多处没有写死（第 6 章四项、第 10 章第 5/7 步）。⚠️ 但也有本以为"没写死"、核对后发现**原文其实给了完整程序**的（第 8 章失败处理，第 77 课）——所以"按原文实现"的第一步是**真的去看原文**，而不是照着流传的转述 |
