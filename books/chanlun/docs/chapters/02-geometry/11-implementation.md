...
| 10 | 笔 | **成笔失败后怎么办** | 丢弃 / 丢弃+同类型取极值 / 回退 | 第 8 章 |
| 11 | 线段 | 特征序列取哪个方向 | 反方向（唯一正确） | 第 10 章 |
| 12 | 线段 | 转折点前后是否做包含处理 | 不做（据转述）/ 做 | 第 10 章 |
...
| 线段 | 判为第二种情况时，缺口未被第 1 元素封闭 | 第 10 章 Q4 |
...
| "只要按原文实现就不会有分歧" | ⚠️ 有争议 | 原表述在多处没有写死（第 6 章四项、第 8 章失败处理、第 10 章第 4/7 步），而本书对第 62 课之后无原文。"按原文"本身就需要先做一批解释性选择 |
