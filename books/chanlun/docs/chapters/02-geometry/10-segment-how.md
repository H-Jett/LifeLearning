...
1. **第二种情况下，即使缺口最终被封闭，也肯定不是被第 1 个元素封闭的**——
   因为那样就变成第一种情况了。
2. **假设的转折点前后那两个元素之间不做包含处理**——
   因为它们已被假设不属于同一性质的东西，不一定属于同一个特征序列；
   而转折点之后的那些元素之间，包含处理照常应用。

第 2 点是本书见过的实现里**出错率最高**的一条。
...
| 4 | **转折点前后两元素不做包含处理** | 全都做了（10.4 节细节 2） |
| 5 | 判断第 1、2 元素间有无缺口 | 判成了第 2、3 元素之间 |
| 6 | 第二种情况下等待反向分型确认 | **直接当第一种处理**（占四成的判定点） |
...
**一、多数细则没有唯一权威答案。** 原表述在第 67 课之后还有第 71 课的再分辨，
说明作者本人也在补充说明；而本书对这几课**没有原文**（见
[出处与资料](../../sources.md)）。在这种情况下声称某套落地"就是正确的"，
本身就缺乏依据。
...
| "有唯一正确的线段划分法" | ⚠️ 缺乏证据 | 第 67 课之后还有第 71 课的再分辨，且本书对这几课无原文。在这种情况下宣称某套落地唯一正确，缺乏依据。本书的立场是标准统一优先于标准正确（10.7 节） |
...
**Q4.**【标签】"第二种情况下，即使缺口最终被封闭，也肯定不是被第 1 个元素封闭的"
属于哪一类命题？请给出理由。
